...
Assigns tasks to memory tiers in arrival order (task_id order).
Fills DRAM first until capacity is exceeded, then assigns remaining tasks to CXL.
"""
...
class FCFSScheduler:
    ...
    This is a simple baseline scheduler that assigns tasks in order of arrival
    (task_id) to memory tiers. It fills DRAM until capacity is exceeded, then
    places remaining tasks in CXL memory.
    ...
    def __init__(self, dram_capacity_mb: float, cxl_capacity_mb: float):
        """
        Initialize FCFS scheduler with memory tier capacities.
    ...
    def schedule(self, tasks: List[Task]) -> Dict[int, str]:
        """
        Schedule tasks using FCFS policy.
        ...
        # 2. Assign to DRAM until capacity exceeded
    ...
    def compute_total_cost(self, tasks: List[Task], assignment: Dict[int, str]) -> float:
        ...
        # Cost = sum(memory_sensitivity × latency_penalty × memory_mb) for CXL tasks
        pass
