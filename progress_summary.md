...
- **Current Branch:** `feature/presentation-enhancements`
...
- **2026-06-15**: Checked out `feature/presentation-enhancements`. Updated the presentation slides: made the Qubit box (Schrödinger's box) lid 90% transparent, increased the visibility of the spinning 0/1 digit, removed navigation buttons in the Architecture step-through walkthrough, and made the architecture step cards clickable to highlight/select steps directly. Removed all visible em dashes from both `presentation/hari/index.html` and `presentation/index.html`. Removed Section 2 (Qubit Demo) entirely from Hari's introduction slide deck as requested, adjusting progress dots, script lists, and sub-card descriptions accordingly. Replaced the video placeholder in the Scale section with the historic Wright brothers first flight image.
